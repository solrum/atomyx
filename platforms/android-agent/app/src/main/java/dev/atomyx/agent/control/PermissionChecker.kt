...
/**
 * Centralized permission state for Atomyx. All checks are pure (no side effects).
 * Use the deepLink* functions to navigate the user to the appropriate Settings page.
 */
object PermissionChecker {

    fun isAccessibilityEnabled(context: Context): Boolean {
        val expected = "${context.packageName}/dev.atomyx.agent.service.AtomyxAccessibilityService"
        return try {
    ...
    fun isAccessibilityServiceConnected(): Boolean =
        dev.atomyx.agent.service.AtomyxAccessibilityService.instance != null
