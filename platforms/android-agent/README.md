...
The host-side `@atomyx/android-driver` TypeScript package connects
...
└── control/
    ...
    ├── GestureRunner.kt              ← completion-aware dispatch pipeline
    │                                   (CountDownLatch 30s, validation,
    │                                   multi-stroke support)
    ├── GestureDispatcher.kt          ← tap / swipe / longPress primitives
    │                                   (single-pointer, delegate to
    │                                   GestureRunner)
    ├── InputDispatcher.kt            ← typeViaKeyboard (+ on-screen-keys
    │                                   fallback for custom Flutter keypads) /
    │                                   clearFocusedInput (+ backspace
    │                                   fallback) / hideKeyboard
    ├── SystemActionDispatcher.kt     ← pressKey / launchApp / forceStopApp
...
| POST | `/actions/clear_focused_input`   | — | Clear the currently focused EditText (ACTION_SET_TEXT with backspace fallback) |
| POST | `/actions/hide_keyboard`         | — | Dismiss the on-screen IME |
...
> `currentForeground()`). The `appId` ↔ `packageName` translation
> happens in `packages/android-driver/src/android.driver.ts` at the
> HTTP boundary so the tool layer never sees Android-specific
> terminology.
...
> **Note on host-side broadening**: the device-side order above is
> the raw strategy chain. The host-side selector-compilation policy
> in `packages/driver/src/selectors/` applies a different priority
> (`id` → `label` → `text` → `hint`) when retrying a failed resolve,
> so the caller gets accessibility-label-first broadening without
> knowing platform conventions.
...
3. Add the client-side call in `packages/android-driver/src/android.driver.ts`.
4. If the capability should reach the tool layer, add a method to
   the `Driver` port in `packages/driver/src/driver/driver.port.ts`
   and implement it in both the Android and iOS adapters.
...
1. Add the field to `SelectorResolver.Selector`.
2. Implement `ResolutionStrategy` in a new file under
   `control/strategy/`.
3. Add it to `SelectorResolver.defaultStrategies()` in the desired
   priority position.
4. Also add the field to the host-side `Selector` type in
   `packages/driver/src/selectors/selector.ts`.
