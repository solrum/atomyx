/**
 * Runtime context for an adet session. Holds all mutable + lifetime state
...
export function createAdetContext(): AdetContext {
  return {
    controller: null,
...
export function requireController(ctx: AdetContext): DeviceController {
  if (!ctx.controller) {
