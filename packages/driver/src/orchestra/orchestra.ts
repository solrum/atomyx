...
import { waitForKeyboard } from "../waits/wait-for-keyboard.js";
import { WaitTimeoutError } from "../waits/wait-until.js";
 ...
 *
 * Feature consumers (MCP server tools, CLI runners, external
 * test-management integrations) never assemble these pieces
 * themselves — they construct one `Orchestra` per session and call
 * its action methods. This keeps the "right way to use the
 * framework" explicit and prevents feature-side drift on how
 * tap-with-selector should actually behave.
 *
 * Contract:
 *
 ...
 *     failure modes: virtualized-list recovery (scroll-search when
 *     the element isn't in the current snapshot) and positional
 *     scroll-into-view with safe-area insets.
 *
...
export class Orchestra {
  ...
  /**
   * Session-level hint: set true after `inputText` completes because
   * typing opens the keyboard on both Android and iOS. Cleared on
   * `hideKeyboard`, `launchApp`, and `stopApp` (any of which reset
   * the keyboard state). Used by `tap()` to decide whether to pay
   * the cost of a keyboard-state RPC — avoids querying on every tap
   * when the session hasn't typed anything yet.
   */
  private maybeKeyboardOpen = false;
  ...
  // Keyboard-dismiss poll budget used by the tap keyboard-gate.
  // Named here so the warn-log message and the waitForKeyboard call
  // share one source of truth; see KEYBOARD_DISMISS_* declarations
  ...
  async tap(selector: Selector): Promise<ActionResult> {
    ...
    if (!prepared.ok) {
      ...
      // Avoids an up-front keyboard RPC on every tap — we only pay
      // the RPC when the normal pipeline already decided we need it.
      if (
        this.maybeKeyboardOpen &&
        ...
        try {
          ...
          this.logger.warn(
            `tap: keyboard did not confirm hidden within ${KEYBOARD_DISMISS_TIMEOUT_MS}ms — tapping anyway`,
          );
        }
  ...
  async inputText(
    ...
    this.maybeKeyboardOpen = true;
    // No Orchestra-level post-type verification either. The agent-
    // level paths already guard silent drops:
    //   - Android `trySetText` verifies ACTION_SET_TEXT took effect
    //     and falls back to per-key keyboard dispatch when it
    //     didn't (handles Flutter obscureText rejections).
    //   - iOS `XCUIApplication.typeText` throws if the keyboard
    //     isn't presentable at dispatch time; silent acceptance
    //     with an empty field doesn't happen.
    // Keeping observation-driven verification at Orchestra would
    // require a fresh hierarchy round-trip per input, and on iOS
    // Flutter the Semantics value lags behind the typeText return
    // so a 1 s verification window falsely flags success as
    // unverified. Downstream script assertions (`waitFor`,
    // `assertVisible`) catch the rare real mis-type cases.
    return okResult({
      resolvedBy: prepared.resolvedBy,
  ...
  /**
   * Dispatch an arbitrary W3C-Actions gesture. Thin passthrough
   * to `Driver.dispatchGesture`; exposed on Orchestra so the
   * YAML `pointer` command and future MCP `pointer` tool share
   * one entry point. Caller is responsible for checking
  ...
  /**
   * The driver's capability flags — proxied so callers (tool
   * layer, script commands) can gate behaviour without reaching
   * into `driver.capabilities` directly. The field is read-only
   * at the port level.
   */
  get capabilities() {
  ...
  /**
   * Resolve a selector to the coordinate the Orchestra's full
   * action pipeline would tap — applying scroll-into-view,
   * obscurement detection, and priority broadening — without
   * actually dispatching a gesture. Intended for callers like
   * the YAML `pointer` compiler that need coordinate-level
   * control over timing but still want the selector safety
   * guarantees of `tap` / `longPress`.
   *
   * Throws on failure (element not found, obscured, scroll
  ...
  async resolvePoint(selector: Selector): Promise<Point> {
    ...
    if (!prepared.ok) {
      ...
      // `reason`, so this narrowing is always true here. The guard
      // is just a type-system formality.
      const reason = !prepared.result.ok
        ? prepared.result.reason
        : "selector resolution failed without a reason";
  ...
  /**
   * Dismiss the on-screen keyboard (if any). Clears the internal
   * `maybeKeyboardOpen` hint regardless of whether the driver
   * actually reported a keyboard — matches the invariant "after
...
// `hideKeyboard` dispatch to be observed in the hierarchy.
const KEYBOARD_DISMISS_TIMEOUT_MS = 1000;
const KEYBOARD_DISMISS_POLL_MS = 50;
