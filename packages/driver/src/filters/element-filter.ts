 ...
 * a list of cursors to a (possibly smaller) list of cursors.
 * Filters compose via `intersect` / `union` / `not` to express
 * arbitrary AND / OR / NOT selector logic.
 *
 * Why pure functions instead of strategy classes:
 *
 *   - Composition. `intersect(idMatches(x), textMatches(y))`
 *     expresses AND, `union` expresses OR, `not` expresses
 *     negation. The priority-broadening ergonomic ("try id, fall
 *     back to text") is a HIGHER-LEVEL policy built on top —
 *     see `selectors/priority-broadening.ts`. Keeping the
 *     primitives orthogonal lets callers that need cross-field
 *     conjunctions express them without fighting a fixed
 *     fallback order.
 *
 *   - Stateless. Unit testing a filter is calling it with a
 *     fixture cursor list and asserting output; no mocking.
 *
 * Filters take and return `TreeCursor[]` rather than `TreeNode[]`
 * so spatial filters (`below`, `hasParent`, …) have access to the
 * parent chain without forcing `parent` onto the wire shape.
 */
