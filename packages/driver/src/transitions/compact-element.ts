 ...
 * The diagnostic functions (loading detection, motion analysis,
 * overlay detection, tree diff) operate on a linear element list,
 * so we flatten here before running them.
 *
 * The framework boundary uses the canonical `TreeNode` tree;
 * `treeNodeToCompactElements` bridges the two at the single call
 * site that needs the flat form.
 */
