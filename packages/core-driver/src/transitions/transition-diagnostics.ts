...
/**
 * Transition diagnostics — ported from the legacy
 * `src/tools/transition-diagnostics.ts` with type adaptation
 * for the new `CompactElement` shape (operates on flat
 * element lists derived from `TreeNode` via
 * `treeNodeToCompactElements`).
 *
 * The pure functions here are used by `TransitionClassifier`
 * (thin class wrapper) and by `tap-and-wait-transition` in
 * `@atomyx/core-driver-mcp` to classify WHY a tap-and-wait
 * attempt failed:
 *
