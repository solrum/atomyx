 ...
 *
 * Replaces the platform-specific `ensureVisible` that previously
 * lived inside `ios-xctest.adapter.ts`. The exact same two-phase
 * algorithm works for both platforms because it operates only on
 * the canonical `TreeNode` shape + the primitive `Driver.swipe()`
 * gesture:
 *
 *   Phase 0 — scroll-search (virtualized list recovery)
 *   ─────────────────────────────────────────────────
 *   When the initial `Finder.find()` returns zero matches, we
 *   assume the target may be recycled off-screen by a virtualized
 *   list (UITableView / UICollectionView on iOS, RecyclerView /
 *   LazyColumn on Android). Swipe the screen center column UP
 *   repeatedly (revealing items logically above — the common case
 *   after a Back navigation leaves the list scrolled to the
 *   bottom), polling the filter after each swipe. If still empty
 *   after the UP budget, reverse direction and swipe DOWN for
 *   another budget.
 *
 *   Phase 1 — positional scroll-into-view
 *   ─────────────────────────────────────
 ...
 *   → scroll content down by swiping finger down; element below
 *   → scroll content up by swiping finger up), execute one
 *   swipe sized to at most `maxScrollFraction * screenHeight`,
 *   re-resolve, loop until visible or bounds stop moving
 *   (progress check — we're at an edge).
 *
...
export interface ScrollControllerOptions {
  ...
  /**
   * Phase 0 budget — swipes in each direction (UP then DOWN)
   * when the target isn't in the current snapshot at all.
...
export class ScrollController {
  ...
  async ensureVisible(filter: ElementFilter): Promise<TreeCursor> {
    ...
    // Phase 0: scroll-search when the initial resolve is empty.
    ...
    // Phase 1: positional scroll-into-view loop.
  ...
  /**
   * Phase 0: alternating-direction probe. Swipes UP for budget
   * iterations, then DOWN for budget iterations, polling the
   * filter between each swipe. Returns the first cursor that
   * matches, or `undefined` if both budgets are exhausted.
   */
  ...
  /**
   * Blind center-column swipe used by Phase 0 scroll-search.
   * Distance is fixed to `maxScrollFraction` — not adaptive,
   * because we don't know where the target is yet.
   */
