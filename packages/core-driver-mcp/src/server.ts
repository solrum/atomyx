...
import type { Logger, Storage, Clock } from "@atomyx/core-driver";
import {
  NoopLogger,
  FileStorage,
  RunStore,
 ...
 *      previous driver and wires a new one. This restores the
 *      pre-refactor legacy MCP server ergonomics on top of the
 *      new per-platform driver packages.
 *
...
export interface McpServerOptions {
   ...
   * `finish_run`) go through this. In-process consumers
   * (Synapse, Studio) can inject a custom `Storage` impl that
   * routes to their own backend.
   */
