...
export class RunStore {
  ...
  recordBug(opts: {
    ...
    // `bugs/<bugId>/screenshot` storage key is a stable sibling
    // of `bugs/<bugId>`. Sequence-based namespacing (run id +
    // per-run counter) also removes any dependency on timestamp
    // uniqueness, so two bugs reported in the same millisecond
