 ...
 *
 * Correctness requires TWO guards that the earlier legacy adapter
 * implementation had to re-discover the hard way:
 *
 *   1. **Port-conflict detection with probe handshake before
 *      spawn.** If `host:port` already has a listener and that
 *      listener answers our ping message, it's an existing adet
