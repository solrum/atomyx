...
/**
 * Buffer cap for `xcodebuild` stdout+stderr. xcodebuild test
 * output is verbose — a clean build easily streams several MB.
 * 10 MiB is large enough for realistic failures without letting
 * a truly runaway subprocess exhaust Node's heap.
 */
const XCODEBUILD_MAX_BUFFER_BYTES = 10 * 1024 * 1024;

/**
 * TCP ping handshake budget used by `probeDriverPing`. Connection
 * is local loopback + iproxy at worst, so 1 s is plenty; longer
 * would delay the "no listener" fallback that kicks off a fresh
 * xcodebuild spawn.
 */
const PING_PROBE_TIMEOUT_MS = 1_000;
...
export interface XctestLauncherOptions {
  ...
  /** Path to `platforms/ios-agent/` (auto-detected from repo root if omitted). */
...
export class XctestLauncher {
      ...
      await new Promise<void>((resolve, reject) => {
        execFile(
          ...
          args,
          {
            cwd: projectDir,
            timeout,
            maxBuffer: XCODEBUILD_MAX_BUFFER_BYTES,
          },
          (err, _stdout, stderr) => {
            if (err) {
  ...
  private async spawnDriver(projectDir: string): Promise<void> {
    const args = [
      ...
      "-derivedDataPath",
      "./build",
      "-only-testing:AtomyxDriverAgent/AtomyxDriverAgent/testServeCommands",
      ...this.signOverrides(),
    ];
...
export function probeDriverPing(port: number): Promise<boolean> {
  return new Promise((resolve) => {
    ...
    sock.once("error", () => finish(false));
    setTimeout(() => finish(false), PING_PROBE_TIMEOUT_MS);
  });
}
